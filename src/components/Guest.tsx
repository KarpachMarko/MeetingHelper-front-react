...
export const Guest = (props: {guest: {name: string, status: number, role: number}, index: number, len: number, opened: boolean, onClick: () => void}) => {
	...
	return (
		<div className="w-full">
			<div onClick={props.onClick} className={`${index === 0 ? "rounded-t-2xl" : ""} ${index === props.len-1 ? "rounded-b-2xl" : ""} cursor-pointer inline-flex relative items-center py-2 px-4 w-full text-3xl font-medium border-b border-gray-200 ${bgColor[guest.status]} hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:border-gray-600 dark:hover:bg-gray-600 dark:hover:text-white dark:focus:ring-gray-500 dark:focus:text-white`}>
