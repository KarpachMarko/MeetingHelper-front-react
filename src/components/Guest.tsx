...
export const Guest = (props: {guest: IGuest, index: number, len: number, opened: boolean, onClick: () => void}) => {
	...
	return (
		<div className="w-full">
			<div onClick={props.onClick} className={`${index === 0 ? "rounded-t-2xl" : ""} ${index === props.len-1 ? "rounded-b-2xl" : ""} cursor-pointer inline-flex relative items-center py-2 px-4 w-full text-3xl font-medium border-b border-gray-200 ${bgColor[guest.priority]} hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:border-gray-600 dark:hover:bg-gray-600 dark:hover:text-white dark:focus:ring-gray-500 dark:focus:text-white`}>
				...
				{guest.user.fullName}
				<span className="absolute bottom-1 right-3 text-indigo-800 text-sm">{guest.role}</span>
			</div>
			<motion.div animate={props.opened ? "opened": "closed"} variants={containerVariants} className="w-full my-2 flex justify-end gap-2 overflow-hidden">
				<a className="font-medium text-sm inline-flex items-center justify-center px-3 py-1 rounded leading-5 bg-gray-200 text-indigo-800 focus:outline-none focus-visible:ring-2"
				   href="#">Message</a>
				{guest.role !== "Creator" ?
					<a className={`font-semibold text-sm inline-flex items-center justify-center px-3 py-1 border border-transparent rounded leading-5 shadow-sm transition duration-150 ease-in-out ${ guest.role === "Responsible" ? "bg-amber-500 hover:bg-amber-600" : "bg-indigo-500 hover:bg-indigo-600"} focus:outline-none focus-visible:ring-2 text-white`}
					   href="#">{guest.role === "Responsible" ? "Remove responsibility" : "Make responsible"}</a> :
					<></>
				}
				<a className="font-semibold text-sm inline-flex items-center justify-center px-3 py-1 border border-transparent rounded leading-5 shadow-sm transition duration-150 ease-in-out bg-red-500 focus:outline-none focus-visible:ring-2 hover:bg-red-600 text-white"
				   href="#">Kick</a>
			</motion.div>
		</div>
	)
